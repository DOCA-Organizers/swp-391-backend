...
public interface MyPostRepository extends JpaRepository<Post,String> {
    ...
    @Query(value = "select * from tblPost where id in (select top 10 postid from tblReact where isactive=1 group by postid order by count(id) desc) and isactive=1 and YEAR(createtime)=?1 and MONTH(createtime)=?2",nativeQuery = true)
